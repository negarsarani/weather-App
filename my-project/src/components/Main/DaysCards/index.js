...
export function DaysCards() {
  const elem = El({
    element: 'div',
    className:
      ' flex flex-col  sm:flex-row items-center justify-center sm:w-full w-5/12 h-full sm:h-fit  ',
    id: 'days-cards',
  });
  FutureData().then((res) => {
...
export function renderDaysCards(item, elem) {
  ...
  const el = El({
    element: 'div',
    className:
      'flex flex-col gap-2 sm:w-10/12 md:w-8/12  lg:w-6/12 sm:flex-row items-center justify-center w-28  h-full  ',
    child: Cards,
  });
  elem.append(el);
}
